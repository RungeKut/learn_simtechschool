...
После загрузки пакета с Apache распакуем загруженный архив. В нем найдем папку непосредственно с файлами веб-сервера - каталог ***Apache24***. Переместим данный каталог на диск C:/webworks/, чтобы полный путь к каталогу составлял ***C:/webworks/Apache24***.
...
Теперь проведем конфигурацию сервера, чтобы связать его с ранее установленным интерпретатором PHP.. Для этого найдем в папке веб-сервера в каталоге conf **(то есть C:/webworks/Apache24/conf )** файл ***httpd.conf***
...
Мы создадим два виртуальных хоста - два сайта которые будут открываться по двум разным адресам.

Прежде всего переопределим директорию по-умолчанию:
    Define SRVROOT "c:/Apache24"

Подключим PHP. Для этого нам надо подключить модуль php, предназначенный для работы с apache. В частности, в папке ***php*** мы можем найти файл ***php8apache2_4.dll***:
...
Теперь заново запустим файл ***httpd.exe*** (лучше запустить файл в консоли набрав **httpd -t** - будет выведен лог ошибок если где-то напутали в конфигах) и обратимся к этому скрипту, набрав в строке браузера адрес:
